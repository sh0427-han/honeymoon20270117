(() => {
    ...
    const renderDocumentStatus = () => {
        ...
        section.innerHTML = `
            ...
            ${driveUrl ? `<a class="more-tool-link" href="${safe(driveUrl)}" target="_blank" rel="noopener noreferrer">Private Drive 열기 ↗</a>` : ""}
        `;
    };
    ...
    const tagExistingSections = () => {
        ...
        const budget = panel.querySelector("#budget-summary")?.closest(".more-section");
        ...
        if (budget) budget.dataset.moreSection = "budget";
    ...
    // PWA/체크리스트가 기존 스크립트에서 다시 그려져도 같은 카테고리를 유지한다.
