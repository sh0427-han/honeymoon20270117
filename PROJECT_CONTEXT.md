...
Public 사이트이므로 예약번호, QR/바코드, 예약확인서 원본, 여권정보, 결제정보 등 민감정보는 커밋하지 않는다.
...
- 예약 카드에는 가격을 표시하지 않음
- 위치는 Google Maps로 연결
- Public 사이트이므로 예약확인서/PDF/QR 이미지는 웹에 보관하지 않음
...
- 별도 App Store / Google Play 링크도 제공
...
Public repository / GitHub Pages에는 아래를 저장하지 않는다.
...
예약 상세정보가 필요하면 웹에 복제하지 않고 로그인된 휴대폰 앱(대한항공 My / Trip.com / Airbnb 등)에서 확인한다.

단, 여행 동선 구현을 위해 일반 공개 장소 수준의 출발지/숙소/관광지 위치와 예약 여부 수준의 비민감 정보는 웹앱에 기록할 수 있다.
